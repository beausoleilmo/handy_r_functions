### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
# Created on Friday, April 22, 2016 at 14:19
# Marc-Olivier Beausoleil 
# Base R PCA funciton that allows a little more flexibility 
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
...
custom_pca <- function(pca = pca, 
                       ...
                       square = FALSE,
                       bg.same.as.col = TRUE,
                       colour.bg.vector = c("#FF3030","#9ACD31","#1D90FF","#FF8001"),
                       vector.to.colour.bg = NULL,
                       square.size = 1,
                       threshold = 0, 
                       pch = 4,
    ...
    # Change the x axis and vectors 
    if(flip.x.axis){
      pca$CA$u[,1] = -pca$CA$u[,1]
      pca$CA$v[,1] = -pca$CA$v[,1]
    }
    # Change the y-axis and vectors 
    if(flip.y.axis){
      pca$CA$u[,2] = -pca$CA$u[,2]
      pca$CA$v[,2] = -pca$CA$v[,2]
    }
  
  if (bg.same.as.col) {
    bg = colour.vector[(as.numeric(vector.to.colour.col))]
  } else {
    bg = colour.bg.vector[(as.numeric(vector.to.colour.bg))]
  }
